...
void evalVariance(Scheme &scheme, Ciphertext &ct, long logp, long len,
                  Ciphertext &ctRes) {
  std::cout << "Compute variance" << std::endl;
  Ciphertext ctMult;
  ctMult = scheme.mult(ct, ct); // Enc((m*p)^2)
  Ciphertext ctRes;
  ctRes = scheme.reScaleBy(ctMult, logp);
  for (int i = 2; i <= len; i *= 2) {
    Ciphertext tmp;
    tmp = scheme.leftRotateFast(ctRes, len / i); // tmp = ciphertext << len/i
...
void evalSigmoid(Scheme &scheme, Ciphertext &ct, long logp, int evalDeg,
                 Ciphertext &ctRes) {
...
void evalExp(Scheme &scheme, Ciphertext &ct, long logp, int evalDeg,
             Ciphertext &ctRes) {
...
// Key Recovery Attack
int attack(Ring ring, long n, long logp, long logq, double ptBound, int evalDeg,
  ...
  int aParity = 0;
  while (!aParity) {
    scheme.encrypt(ctxt_input, val_input, n, logp,
                   logq); // Enc(m*p) for scaling factor p

    switch (hc) {
    case HC_VARIANCE:
      evalVariance(scheme, ctxt_input, logp, n,
                   ctxt_res); // compute sum(m^2 for m in val_input)
      break;
    case HC_SIGMOID:
  ...
  std::vector<complex<double>> heRes(val_res, val_res + n),
      ptIn(val_input, val_input + n), ptRes(n);
  switch (hc) {
  case HC_VARIANCE:
  ...
  case HC_EXP:
    ...
    break;
  default:
    ptRes.assign(val_input,
                 val_input + n); // noop, just copy the plaintext input
  }
  ...
  ZZ ql =
      power2_ZZ(ctxt_res.logq); // The modulus of current level (in ctxt_res)
  std::cout << "encoding error = " << maxDiff(ptxt_res.mx, error.mx, n, ql)
            << std::endl
...
int main(int argc, char *argv[]) {
  ...
  for (int i = 0; i < iter; i++) {
    std::cout << "Running attack on " << hcString(hc) << std::endl
              << " N = " << N << ", logq = " << logq << ", logp = " << logp
              << ", |plaintext| = " << plainBound << std::endl;
